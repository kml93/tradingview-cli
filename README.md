...
CLI and Claude Code skills for TradingView Desktop via Chrome DevTools Protocol.
...
```bash
bun install --global "github:kml93/tradingview-skills"
```
...
Skills are installed automatically via `postinstall` to `~/.config/claude/skills/`. No manual setup needed.
...
Based on [tradingview-mcp](https://github.com/tradesdontlie/tradingview-mcp) by tradesdontlie.
